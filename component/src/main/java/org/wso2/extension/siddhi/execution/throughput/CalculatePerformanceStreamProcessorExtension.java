...
@Extension(
        ...
        description = "Measuring performance of stream processor with simple passthrough", //todo: elaborate description
        parameters = {
                @Parameter(name = "iijtimestamp",
                           description = "This value used to find the sending timestamp from client",
                           type = {DataType.LONG}),
                @Parameter(name = "value",
                           description = "This value used to find the generated random number",
                           type = {DataType.FLOAT}),
        },
        examples = {
                @Example(
...
public class CalculatePerformanceStreamProcessorExtension extends StreamProcessor {
    ...
    private static String logDir = "./TCP-Results";
    private static final int RECORDWINDOW = 10000000;
    ...
    protected List<Attribute> init(AbstractDefinition inputDefinition, ExpressionExecutor[]
        ...
        if (attributeExpressionLength == 3) {
            ...
            if (!(attributeExpressionExecutors[1] instanceof VariableExpressionExecutor)) {
                throw new SiddhiAppValidationException("value has to be a variable but found " +
                                                               this.attributeExpressionExecutors[0].getClass()
                                                                       .getCanonicalName());
            }
            ...
            if (attributeExpressionExecutors[1].getReturnType() == Attribute.Type.FLOAT) {
                ...
                throw new SiddhiAppValidationException("value expected to be float but "
                                                               + "found" + attributeExpressionExecutors[1]
                        .getReturnType());
            }

            if (!(attributeExpressionExecutors[2] instanceof ConstantExpressionExecutor)) {
                throw new SiddhiAppValidationException("Third parameter has to be constant but found" + this
            ...
            if (attributeExpressionExecutors[2].getReturnType() == Attribute.Type.STRING) {
                executionType = (String) ((ConstantExpressionExecutor) attributeExpressionExecutors[2]).getValue();
            } else {
                throw new SiddhiAppValidationException("Third parameter expected to be String but "
                                                               + "found" + attributeExpressionExecutors[2]
                        .getReturnType());
            }
            ...
            while (streamEventChunk.hasNext()) {
                ...
                try {
                    ...
                    if (eventCount >= RECORDWINDOW) {
                        ...
                        log.info("Total time spent" + totalTimeSpent);
                        ...
                        if (!exitFlag && eventCountTotal == 100000000L) {
                    ...
                    if (eventCount >= RECORDWINDOW) {
                        ...
                        if (!flag) {
                            ...
                            fstream.write("Id, Throughput in this window (thousands events/second), Entire "
                                                  ...
                                                  + "elapsed time(s),EventCountPerWindow,Total Events,Record "
                                                  + "Window,FirstTupleTime,Value,CurrentTime");
                            fstream.write("\r\n");
                        }
                    ...
                    if (eventCount == RECORDWINDOW) {
                        ...
                        if (!flag) {
                            ...
                            fstream.write("Id, Throughput in this window (thousands events/second), Entire "
                                                  + "throughput for the run (thousands events/second), Total "
                                                  + "elapsed time(s),EventCountPerWindow,Total Events,Record "
                                                  + "Window,FirstTupleTime,Value,CurrentTime,Average "
                                                  + "latency "
                                                  +
                                                  "per event in this window(ms), Entire Average latency per "
                                                  + "event for the run(ms), Total "
                        ...
                        long time = timeSpent;
                        long totalTime = totalTimeSpent;
//                        log.info("event count" + eventCount + "," + "currentTime" + currentTime + "value" + value
//                                         + "time" + time);
